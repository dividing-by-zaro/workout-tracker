...
- **BackgroundAudioService**: Plays silent audio to keep the app process alive in background (required for rest timer expiry and Live Activity updates). Also plays `alert_tone.caf` via `playAlertSound()` on timer expiry — the active `.playback` session suppresses `AlertConfiguration(sound:)`, so the alert must go through `AVAudioPlayer` directly. `isPlaying` is a computed property checking `audioPlayer?.isPlaying` (not a manual bool) so `handleForegroundResume` can detect a dead player. Observes `AVAudioSession.interruptionNotification` to auto-restart silent audio after interruptions (e.g. another app's audio session taking over). The `shouldBePlaying` flag gates restart to only when a workout is active. Without this interruption handling, music apps would kill the silent player, iOS would suspend the app, and rest timer expiry + Live Activity updates would silently fail for the remainder of the workout.
